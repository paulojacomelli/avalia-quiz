                ...
                case "vertex":
                    ...
                    if (!gRes.ok) {
                        ...
                        if (gRes.status === 404 || errTxt.toLowerCase().includes("no longer available") || errTxt.toLowerCase().includes("not found")) {
                            ...
                            throw new https_1.HttpsError("invalid-argument", `O modelo '${testModel}' foi descontinuado pelo provedor (Shutdown). Selecione outro modelo.`);
                        }
                        if (gRes.status === 429) {
                            throw new https_1.HttpsError("resource-exhausted", `Limite de requisições ou cota temporária excedida para o modelo '${testModel}' (HTTP 429). Aguarde alguns instantes ou selecione outro modelo.`);
                        }
                        if (gRes.status === 503 || errTxt.toLowerCase().includes("high demand")) {
                            throw new https_1.HttpsError("unavailable", `O servidor do modelo '${testModel}' está temporariamente sob alta demanda (HTTP 503). Tente novamente em alguns segundos.`);
                        }
                        throw new https_1.HttpsError("invalid-argument", `O modelo '${testModel}' do Google AI não está operacional no momento (HTTP ${gRes.status}).`);
                    }
    ...
    try {
        ...
        if (provider === "google-ai" || provider === "vertex") {
            ...
            if (apiData && Array.isArray(apiData.models)) {
                ...
                const activeDefault = (defaultModel && result.some((m) => m.value === defaultModel && !m.isBlocked)) ? defaultModel : '';
