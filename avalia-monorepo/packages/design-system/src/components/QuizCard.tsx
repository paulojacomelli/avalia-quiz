...
export const QuizCard: React.FC<QuizCardProps> = ({ 
  ...
  useEffect(() => {
    return () => {
      if (notificacaoTimeoutRef.current) clearTimeout(notificacaoTimeoutRef.current);
    };
  }, []);

  const exibirNotificacao = (mensagem: string, tipo: 'erro' | 'info' = 'erro') => {
    if (notificacaoTimeoutRef.current) clearTimeout(notificacaoTimeoutRef.current);
  ...
  useEffect(() => {
    if (isAskLoading || isEvaluating || isSkipping) startLoadingDrone();
    else stopLoadingDrone();
  }, [isAskLoading, isEvaluating, isSkipping]);

  // --- Live Mode: Gemini Live API ---
  const startLiveSession = () => {
    if (!apiKey) {
      exibirNotificacao('API Key necessária para o modo Live.', 'info');
      return;
    }
    if (liveSessionRef.current) { liveSessionRef.current.stop(); liveSessionRef.current = null; }
    ...
    const session = new LiveApiSession({
      ...
      onEvaluationReady: async (transcript) => {
  ...
  }, [question.id, openEndedMode]);

  useEffect(() => {
    if (isTimeUp && !hasConfirmed && !showAnswerKey) handleConfirm();
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const mapping: Record<string, number> = { '1': 0, 'a': 0, '2': 1, 'b': 1, '3': 2, 'c': 2, '4': 3, 'd': 3 };
      if (mapping[key] !== undefined && mapping[key] < question.options.length) handleSelect(mapping[key]);
    };
  ...
  const toggleRecording = () => {
    ...
    if (!SpeechRecognition) {
      exibirNotificacao('Seu navegador não suporta entrada de voz.', 'info');
      return;
    }
  ...
  const handleMainHelpClick = () => {
    ...
    if (allowStandardHint && allowAskAi) {
      ...
      playSound('click');
    } else if (allowStandardHint) activateStandardHint();
    else if (allowAskAi) activateChat();
  };
  ...
  return (
    <div className="w-full max-w-3xl mx-auto flex flex-col h-full justify-center relative p-2 md:p-0">
      ...
      {activeTeamName && !showAnswerKey && (
        <div className="mb-4 inline-block self-start text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider animate-fade-in shadow-sm"
      ...
      <div className="mb-6 flex items-start gap-4 group/header relative z-10">
        ...
        <div className="flex-1">
          ...
          {isAnsweredOrFinished && <span className="block mt-2 text-sm opacity-50 italic animate-fade-in">Ref: {question.reference}</span>}
              ...
              <div className="flex flex-col items-center justify-center py-8 gap-6">
                ...
                {livePhase === 'speaking' && (
                  <div className="flex flex-col items-center gap-3 animate-fade-in">
                    ...
                    <p className="text-sm text-gray-400">Lendo a pergunta...</p>
                    {liveModelTranscript && (
                      <p className="text-xs text-gray-500 italic max-w-xs text-center">{liveModelTranscript}</p>
                    )}
                  </div>
                )}
                {livePhase === 'listening' && (
                  <div className="flex flex-col items-center gap-4 animate-fade-in w-full">
                    <div className="w-16 h-16 rounded-full bg-red-600/10 border-2 border-red-500 flex items-center justify-center animate-pulse">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className="w-8 h-8 text-red-400">
      ...
      {!isAnsweredOrFinished && (
        <div className="pl-0 md:pl-8 mt-6 relative z-10">
          <div className="flex items-center gap-2 md:gap-4 flex-wrap">
            ...
            {hintsRemaining !== -1 && <span className="text-xs font-mono opacity-50 ml-auto md:ml-0">Dicas: {hintsRemaining}</span>}
          </div>
          {activeHint && (
            <div className="w-full mt-4 text-sm text-blue-300 bg-blue-900/20 p-4 rounded-lg border border-blue-900/50 animate-fade-in relative">
              <button onClick={() => setActiveHint(null)} className="absolute top-2 right-2 opacity-50 hover:opacity-100 p-1" title="Fechar dica">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
              <span className="font-bold mr-1 block mb-1 text-blue-200">Dica:</span> {activeHint}
            </div>
          )}
        </div>
      )}
