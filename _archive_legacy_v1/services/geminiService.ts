...
export const evaluateFreeResponse = async (apiKey: string, question: string, modelAnswer: string, userAnswer: string): Promise<EvaluationResult> => {
  ...
  if (!raw) throw new Error("Resposta vazia da IA ao avaliar resposta livre.");
  try {
    return JSON.parse(cleanJson(raw)) as EvaluationResult;
  } catch (e) {
    throw new Error("Erro ao processar avaliação da IA: resposta não é um JSON válido.");
  }
};
