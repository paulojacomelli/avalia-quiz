...
// --- TOUR CONFIGURATION (fora do componente para evitar re-criação a cada render) ---
const createTourSteps = (setSetupStep: (s: number) => void): TourStep[] => [
  {
...
function App() {
  ...
  // Audio/TTS Refs — nenhum ref de áudio ativo no momento

  // TOUR_STEPS construído via factory (definida fora do componente, evita re-criação a cada render)
  const TOUR_STEPS = useMemo(() => createTourSteps(setSetupStep), []);
  ...
  const handleGenerate = async (config: QuizConfig) => {
    ...
    try {
      ...
      if (isPrebuiltQuiz) {
        ...
        // Fire-and-forget intencional: falhas de salvamento não impactam o fluxo da partida.
        void saveGeneratedQuiz(data, "Geral Quiz", finalConfig.mode, finalConfig.subTopic || finalConfig.specificTopic);
      }
  ...
  const handleSkipQuestion = async () => {
    ...
    try {
      ...
      // Verifica localmente se é erro de quota para não resetar isSkipping com valor stale de closure
      ...
      handleApiError(e);
      if (!isQuotaError) setIsSkipping(false);
    } finally {
      // isSkipping só é resetado aqui se não for erro de quota (controlado pelo catch acima)
      // Nota: não usar cooldownTime aqui — valor stale em closures assíncronas
    }
  };
